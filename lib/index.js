...
*/
const BILLING_STATUS_PATH = "/console/api/billing/status";
...
async function fetchViaCookie(cfg) {
	...
	try {
		const [status, billing] = await Promise.all([getJSON(cfg, GO_STATUS_PATH, controller.signal), getJSON(cfg, BILLING_STATUS_PATH, controller.signal).catch(() => void 0)]);
...
*/
function fromBillingJSON(payload) {
...
function toView(data) {
	return {
		...
		...data.credit === void 0 ? {} : { credit: data.credit }
	};
}
//#endregion
//#region src/types.ts
...
//#endregion
export { BILLING_STATUS_PATH, CONSOLE_COOKIE_NAMES, GO_STATUS_PATH, MICROCENTS_PER_USD, OCGO_API_PREFIX, OcgoUsageService, UsageError, WORKSPACE_HEADER, apply, configFilePath, fetchUsage, fromBillingJSON, fromStatusJSON, inject, loadConfig, makeOcgoRoutes, name, normalizeCookie };
