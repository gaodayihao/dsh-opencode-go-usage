...
function memberOf(literal, key) {
    const pattern = new RegExp(`["']?\\b${key}\\b["']?\\s*:\\s*("(?:[^"\\\\]|\\\\.)*"|'(?:[^'\\\\]|\\\\.)*'|!?[A-Za-z_$][\\w$]*|-?\\d+(?:\\.\\d+)?(?:[eE][+-]?\\d+)?)`);
...
function labelToKind(label) {
    ...
    if (lower.startsWith('滚动') || lower.includes('小时'))
        return 'rolling';
    if (lower.startsWith('每周') || lower.includes('周'))
        return 'weekly';
    if (lower.startsWith('每月') || lower.includes('月'))
        return 'monthly';
    return undefined;
}
