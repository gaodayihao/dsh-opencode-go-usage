...
import { isOpenCodeGo, providerOfModelSelection } from "../provider.js";
import { MICROCENTS_PER_USD, } from "../types.js";
 ...
 */
export function remainingSec(window) {
 ...
 */
export function hasReset(window) {
 ...
 */
export function formatSpend(window) {
    if (window.usage === undefined || window.limit === undefined)
        return undefined;
...
function severityClass(window) {
    ...
    if (window.percent >= 60)
        return css.segWarn60;
    if (window.percent >= 50)
...
function WindowSegment(props) {
    ...
    return (_jsxs("span", { className: css.seg, children: [_jsx("span", { className: css.segSep, children: sep }), _jsxs("span", { className: cls ?? undefined, children: [WINDOW_LABELS[window.kind], " ", formatPercent(window.percent), "%", !compact ? ` (${formatDuration(window.resetInSec)})` : ''] })] }));
}
...
export function OcgoDockEntry(props) {
    ...
    if (error !== null) {
        return (_jsxs("span", { className: css.wrap, ref: wrapRef, "data-testid": "ocgo-chip-error", children: [_jsxs("button", { type: "button", className: open ? `${css.chip} ${css.chipOpen}` : css.chip, onClick: () => { if (open)
                        ...
                        openSet(); }, title: `${error.message}\n${t('ocgo.set')}`, children: [_jsx(OcgoLogo, {}), " <err:", error.code, ">"] }), open && (_jsx("span", { className: css.details, children: _jsxs("span", { className: css.setPanel, children: [_jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('ocgo.workspaceID') }), _jsx("input", { className: css.fieldInput, value: wsDraft, placeholder: "wrk_\u2026", spellCheck: false, autoComplete: "off", onChange: (e) => { setWsDraft(e.target.value); }, onFocus: (e) => { if (e.target.value === maskedText(config?.workspaceID))
                                            e.target.select(); } })] }), _jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('ocgo.cookie') }), _jsx("input", { className: css.fieldInput, value: cookieDraft, placeholder: "__Host-console_session=\u2026", spellCheck: false, autoComplete: "off", onChange: (e) => { setCookieDraft(e.target.value); }, onFocus: (e) => { if (e.target.value === maskedText(config?.cookie))
    ...
    return (_jsxs("span", { className: css.wrap, ref: wrapRef, "data-testid": "ocgo-chip", children: [_jsxs("button", { type: "button", className: open ? `${css.chip} ${css.chipOpen}` : css.chip, onClick: () => { if (open)
                    ...
                    setOpen(true); }, title: open ? t('ocgo.collapse') : t('ocgo.expand'), children: [_jsx(OcgoLogo, {}), windows.map((w) => (_jsx(WindowSegment, { window: w, sep: sep, compact: true }, w.kind))), snapshot.credit !== undefined && (_jsxs("span", { className: css.seg, "data-testid": "ocgo-chip-credit", children: [_jsx("span", { className: css.segSep, children: sep }), _jsxs("span", { children: [t('ocgo.creditShort'), " ", formatCredit(snapshot.credit)] })] })), _jsx("span", { className: open ? `${css.chevron} ${css.chevronOpen}` : css.chevron, "aria-hidden": "true", children: _jsx("svg", { width: "12", height: "12", viewBox: "0 0 12 12", fill: "none", children: _jsx("path", { d: "M3 4.5L6 7.5L9 4.5", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round", strokeLinejoin: "round" }) }) })] }), open && (_jsx("span", { className: css.details, children: mode === 'set' ? (_jsxs("span", { className: css.setPanel, children: [_jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('ocgo.workspaceID') }), _jsx("input", { className: css.fieldInput, value: wsDraft, placeholder: "wrk_\u2026", spellCheck: false, autoComplete: "off", onChange: (e) => { setWsDraft(e.target.value); }, onFocus: (e) => { if (e.target.value === maskedText(config?.workspaceID))
                                        e.target.select(); } })] }), _jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('ocgo.cookie') }), _jsx("input", { className: css.fieldInput, value: cookieDraft, placeholder: "__Host-console_session=\u2026", spellCheck: false, autoComplete: "off", onChange: (e) => { setCookieDraft(e.target.value); }, onFocus: (e) => { if (e.target.value === maskedText(config?.cookie))
                                        e.target.select(); } })] }), _jsxs("span", { className: css.foot, children: [_jsx("span", { className: css.setHint, children: t('ocgo.setHint') }), _jsx("button", { type: "button", className: css.refreshBtn, onClick: closePanel, children: t('ocgo.save') })] })] })) : (_jsxs(_Fragment, { children: [windows.map((w) => {
                            ...
                            return (_jsxs("span", { className: css.window, children: [_jsx("span", { className: css.windowLabel, children: w.status === 'rate-limited' ? t('ocgo.rateLimited') : t(WINDOW_TITLE_KEYS[w.kind]) }), _jsxs("span", { className: css.windowValue, children: [_jsxs("span", { className: severityClass(w) ?? undefined, children: [formatPercent(w.percent), "%"] }), detail === '' ? null : _jsx("span", { className: css.windowReset, children: detail })] })] }, w.kind));
                        }), snapshot.credit !== undefined && (_jsxs("span", { className: css.window, "data-testid": "ocgo-credit", children: [_jsx("span", { className: css.windowLabel, children: t('ocgo.credit') }), _jsx("span", { className: css.windowValue, children: _jsx("span", { className: css.creditValue, children: formatCredit(snapshot.credit) }) })] })), _jsxs("span", { className: css.foot, children: [_jsx("button", { type: "button", className: css.setBtn, onClick: openSet, children: t('ocgo.set') }), _jsxs("span", { className: css.footRight, children: [_jsx("button", { type: "button", className: css.refreshBtn, onClick: refresh, children: t('ocgo.refresh') }), snapshot.updatedAt !== undefined && (_jsx("span", { className: css.fetchedAt, children: t('ocgo.fetchedAt', { time: formatClock(snapshot.updatedAt) }) }))] })] })] })) }))] }));
}
