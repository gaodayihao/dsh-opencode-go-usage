...
const METER_KEYS: Record<UsageWindowKind, string> = {
  rolling: 'fiveHour',
  weekly: 'week',
  monthly: 'month',
}
...
const KINDS = ['rolling', 'weekly', 'monthly'] as const
...
function transportError(error: unknown, timeoutMs: number): UsageError {
...
function statusError(status: number, path: string): UsageError {
  ...
  if (status === 401) {
    return new UsageError(
      'The console session cookie was rejected; sign in to opencode.ai again and paste the fresh cookie.',
      'unauthorized',
    )
  }
  if (status === 404) {
    return new UsageError(
      'No Go subscription found for this workspace (unknown workspace id, or the plan is not on this account).',
      'notfound',
    )
  }
 ...
 */
export function fromStatusJSON(
  payload: unknown,
  now: number = Date.now(),
 ...
 */
function secondsUntil(at: string | undefined, now: number): number {
